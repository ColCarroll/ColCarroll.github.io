...
function randomFunction() {
    var fn = randomProperty({
        ...
        "|\\cos{(5x)}|": function(x) {
            return Math.abs(Math.cos(5 * x))
        },
        "e^x": function(x) {
            return Math.exp(x)
        },
        "\\frac{1}{1 + e^{-9x}}": function(x) {
            return 1.0 / (1 + Math.exp(-9 * x))
        },
        "\\sin{\\frac{1}{x}}": function(x) {
    ...
    var trainingPoints = 10 + Math.abs(Math.round(Math.random() * 200)),
        ...
        y = x.map(fn[1]),
        noise = 0.6 * Math.abs(Math.random()),
        random = d3.randomNormal(0, noise),
        yNoise = y.map(function(d) {
            return d + 2 * random();
        });
